      ...
      <CardContent>
        <form className="grid gap-5" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-2">
            ...
            <Input id="business-name" value={formData.businessName} onChange={(event) => handleChange("businessName", event.target.value)} disabled={!canEdit} />
          </div>

          <div className="grid gap-2 sm:grid-cols-2">
            <div className="grid gap-2">
              <Label htmlFor="business-email">Business email</Label>
              <Input id="business-email" type="email" value={formData.email} onChange={(event) => handleChange("email", event.target.value)} disabled={!canEdit} />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="business-phone">Phone number</Label>
              <Input id="business-phone" value={formData.phone} onChange={(event) => handleChange("phone", event.target.value)} disabled={!canEdit} />
            </div>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Input id="business-gstin" value={formData.gstin} onChange={(event) => handleChange("gstin", event.target.value)} disabled={!canEdit} />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="business-website">Website</Label>
              <Input id="business-website" value={formData.website} onChange={(event) => handleChange("website", event.target.value)} disabled={!canEdit} placeholder="https://example.com" />
            </div>
          </div>

          <div className="grid gap-2">
            <Label htmlFor="business-tagline">Tagline</Label>
            <Input id="business-tagline" value={formData.tagline} onChange={(event) => handleChange("tagline", event.target.value)} disabled={!canEdit} placeholder="Short brand line or description" />
          </div>

          <div className="grid gap-2">
            <Label htmlFor="business-address">Address</Label>
            <Textarea id="business-address" value={formData.address} onChange={(event) => handleChange("address", event.target.value)} disabled={!canEdit} rows={5} />
          </div>
